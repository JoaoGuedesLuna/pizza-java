...
/**
 * Essa classe representa a janela de início onde o usuário poderá ver todos os produtos disponíveis.
 * 
 * @author João Guedes.
 */
public final class HomeFrame extends NavigationFrame {
    
    private static List<Item> items;
    
    public HomeFrame(Customer customer, Contact contact, Address address, Account account) {
        super();
        super.setCustomer(customer);
        super.setContact(contact);
        super.setAddress(address);
        super.setAccount(account);
        super.setCart(new LinkedList<>());
        this.fillItemsList();
        this.initComponents();
    ...
    private void initComponents() {
        ...
        rightPanel.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(51, 51, 51), 35));
        rightPanel.setLayout(new java.awt.GridLayout(6, 2, 30, 30));
    ...
    /**
     * Faz o preenchimento da lista de items disponíveis para compra de acordo com a quantidade de produtos disponíveis no
     * banco de dados.
     */
    private void fillItemsList() {
        try {
            List<Product> products = FactoryDAO.createProductDAO().findAll();
        ...
        } catch (DataBaseException ebe) {
            super.clear();
            Screen.changeScreen(this, new LoginFrame());
            JOptionPane.showMessageDialog(null, "Desculpe, não foi possível conectar ao servidor. Verifique sua conexão.", 
                    "Aviso", javax.swing.JOptionPane.INFORMATION_MESSAGE);
        }
    }
    
    /**
     * Método responsável por carregar todos os anúncios de produto de acordo com a lista de produtos.
     */
    private void renderAds() {
        AdPanel adPanel;
        for (Item item : items) {
            adPanel = new AdPanel(this, item, super.getCart().contains(item));
            this.rightPanel.add(adPanel);
        }
    }
